...
export const withAirtableContext = Component =>
  class Higher extends React.Component {
    render() {
      return (
        <Consumer>{state => <Component {...this.props} {...state} />}</Consumer>
      );
    }
  };
...
class AirtableProvider extends React.PureComponent {
  ...
  fetchNewAssetListing = async (network, updateFunction, assetId) => {
    let assetListings = await fetch(
      InternalLinks.getAirtableAssetListings(network)
    );
    assetListings = await assetListings.json();
    const { records: listingsRecords } = assetListings;
    const assetListingsFiltered = verifyDataAirtable(
      AIRTABLE_ASSET_LISTINGS,
      listingsRecords
    );
    const assetListingsProcessed = this.processAssetListings(
      assetListingsFiltered
    );
    if (assetListingsProcessed[assetId]) {
      ...
      setTimeout(
        () => this.fetchNewAssetListing(network, updateFunction, assetId),
        200
      );
    }
  };
  ...
  getAssets = async (network, updateFunction) => {
    try {
      ...
      let assetListings = await fetch(
        // InternalLinks.getAirtableAssetListings(network)
        "https://mybit-goapp-ghw2l1433.vercel.app/api/airtable/assets/mainnet"
      );
      assetListings = await assetListings.json();
      const { records: listingsRecords } = assetListings;

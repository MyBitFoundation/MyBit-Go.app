...
const CustomTimeline = React.memo(
  ({
    ...
    return (
      <CustomTimelineWrapper listedAssetId={listedAssetId}>
        <CustomTimelineItem
          title="Asset"
          content="Pick the asset you will be managing."
          step={1}
          currentStep={step}
        ...
        <CustomTimelineItem
          ...
          content={
            step > 2
              ? "Provided information about the asset, financial aspects and associated risks."
            ...
            step > 4 ? (
              <div>
                {fileList.length === 0
                  ...
                  : fileList.map(file => <div>{file.name}</div>)}
              </div>
            ) : (
        ...
        <CustomTimelineItem
          ...
          content={
            step > 5
              ? `${managementFee}%`
              : "How much will it cost for you to operate the asset?"
          }
          step={5}
          currentStep={step}
          ...
          content={
            step > 6 ? (
              collateralPercentage === 0 ? (
                ...
                <div>
                  <div>
                    {`${collateralPercentage}% of the asset = ${formatMonetaryValue(
                      collateralInPlatformToken,
                      PLATFORM_TOKEN
                    )}`}
                  </div>
        ...
        <CustomTimelineItem
          ...
          content={
            listedAssetId
              ...
              : "Check if everything is right, confirm and deposit collateral with MetaMask."
          }
          step={!readToS ? 8 : 7}

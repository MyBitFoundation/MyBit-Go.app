    ...
    return (
      <CirclesBackgroundWrapper>
        <div className="HelpPage">
          ...
          <p className="HelpPage__faucet-text">Don’t forget to get your faucet MYB here.</p>

          {!this.state.sendingMyb &&
            <Button small kind="primary" className="" onClick={() => this.handleGetMYB()}>
              Send me the cash
            </Button>
          }
          {this.state.transactionStatus === 1 && <p className="HelpPage__faucet-status-text">Sent successfuly!</p>}
          {this.state.transactionStatus === 0 && <p className="HelpPage__faucet-status-text">Transaction failed, please try again.</p>}
          {this.state.sendingMyb &&
            <div>
              <Loading
                small
                ...
                style={{ margin: '0 auto' }}
              />
              <p>Accept the transaction in metamask and wait for a brief moment.</p>
            </div>
          }
